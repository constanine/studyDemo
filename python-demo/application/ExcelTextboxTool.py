...
def getXlsTexts(this):
    ...
    return ExcelXmLParser(s.read(p)).text;
...
class ExcelXmLParser(object):
    ...
    def tag_content(self, tag):
        return [ExcelXmLParser(i) for i in self.value.split(tag)[1::2]]
    ...
    def text(self):
        ...
        return [i for j in l for i in j]  #  join sublists of lines into strings seperated by newline char